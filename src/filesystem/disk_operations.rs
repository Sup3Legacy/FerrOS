...
pub fn init() {
    unsafe {
        ...
        // /!\ Should not to this if it was disabled before !
        wait_BSY(DISK_PORT);
    }
}
...
unsafe fn read(table: *mut [u16; 256], lba: u32, port: u16) {
    ...
    wait_BSY(port);
    let lba = (lba & 0xFFFF) as u64;
    let mut data_register = Port::<u16>::new(port + 0);
    let mut error_register = Port::<u8>::new(port + 1);
    ...
    lba_low.write(((lba >> 24) & 0x0F) as u8);
    lba_mid.write(((lba >> 32) & 0x0F) as u8);
    lba_high.write(((lba >> 40) & 0x0F) as u8);
    sectorcount_register.write(1_u8); // says that we want to read only one register
    lba_low.write(((lba >> 0) & 0x0F) as u8);
    lba_mid.write(((lba >> 8) & 0x0F) as u8);
    lba_high.write(((lba >> 16) & 0x0F) as u8);
...
pub fn write_sector(lba: u32, table: &[u16; 256]) {
    if lba < 512 {
        println!("writing to sector at lba : {} = {}", lba, lba as u64);
    }
    unsafe {
...
unsafe fn write(table: &[u16; 256], lba: u32, port: u16) {
    ...
    if lba < 512 {
        println!(
            "Finally writing to sector at lba : {} = {}",
            lba, lba as u64
        );
    }
    let lba = lba as u64;
    ...
    drive_head_register.write(0x40_u8); // outb(0x1F6, 0xE0 | (slavebit << 4) | ((LBA >> 24) & 0x0F))
    sectorcount_register.write(0_u16); // says that we want to read only one register
    lba_low.write(((lba >> 24) & 0x0F) as u8);
    lba_mid.write(((lba >> 32) & 0x0F) as u8);
    lba_high.write(((lba >> 40) & 0x0F) as u8);
    sectorcount_register.write(1_u16); // says that we want to read only one register
    lba_low.write((lba & 0x0F) as u8);
    lba_mid.write(((lba >> 8) & 0x0F) as u8);
    lba_high.write(((lba >> 16) & 0x0F) as u8);
